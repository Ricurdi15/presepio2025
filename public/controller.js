...
function checkOrientation() {
    ...
    // Usa window.matchMedia per iOS - più affidabile
    ...
    if (gameStarted && isSmallScreen && isPortrait) {
        ...
        // Pausa il gioco mentre è in portrait
        if (gameRunning) {
            gameRunning = false;
            // Salva che era in pausa per portrait
            window.pausedForRotation = true;
        }
    } else {
        rotateMsg.classList.remove('show', 'mobile-portrait');
        // Riprendi se era in pausa per rotation
        if (window.pausedForRotation && gameStarted) {
            gameRunning = true;
            window.pausedForRotation = false;
        }
    }
}
...
window.addEventListener('resize', () => {
    // Delay più lungo per iOS
    setTimeout(() => { resizeCanvas(); checkOrientation(); }, 150);
});
resizeCanvas();
checkOrientation();
gameLoop();